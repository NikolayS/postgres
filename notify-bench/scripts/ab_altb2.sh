...
verify_arm() {   # $1 = expected arm
  ...
  for i in 1 2 3 4 5; do
    n=$((n + $(psql -U postgres -tAc "select count(*) from pg_stat_activity where (wait_event_type='Lock' and wait_event='object') or wait_event='NotifyQueueInsert'" 2>/dev/null)))
    sleep 0.3
  done
  wait $bp
...
for c in $CLIENTS; do
  for r in $(seq 1 $REPS); do
    ...
    run /tmp/ab_notify.sql  "$c" notify_altb2 "$r"
  done
done
